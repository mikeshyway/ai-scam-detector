...
If this folder is inside a GitHub repository root, set the Streamlit app path to:

```text
ai-scam-detector/app/main.py
```

Keep a `requirements.txt` file at the repository root. This repository includes a root
`requirements.txt` that forwards to `ai-scam-detector/requirements.txt`, which lets Streamlit
Cloud install packages such as `plotly`, `matplotlib`, `scikit-learn`, and `librosa`.
