...
The project uses simple, explainable preprocessing because the capstone goal is to teach
students how scam detection works, not to hide the pipeline inside a large black-box model.
"""
...
def _nltk_stopwords() -> set[str]:
    """Return NLTK stopwords when installed, otherwise use scikit-learn's list."""
...
def clean_text(text: object, *, remove_stopwords: bool = True, lemmatize: bool = False) -> str:
    """Normalize text while preserving useful scam markers such as URLs and money amounts."""
...
def build_tfidf_vectorizer(max_features: int = 8000) -> TfidfVectorizer:
    """Create the TF-IDF vectorizer used by email and transcript models."""
...
def extract_email_body(path: Path) -> str:
    """Extract readable text from a raw email file."""
...
def load_spamassassin_dataset(raw_dir: str | Path) -> pd.DataFrame:
    """Load SpamAssassin-style `spam/` and `ham/` folders into a dataframe."""
...
def label_to_binary(value: object) -> int:
    """Convert common dataset label values into 0=legitimate/real and 1=scam/fake."""
...
def load_labeled_text_csv(
    ...
    *,
    ...
    """Load a labeled text CSV for transcript scam classification."""
